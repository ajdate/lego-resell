...
const CONDITIONS: { value: SimulationCondition; label: string }[] = [
  { value: "sealed", label: "Sealed" },
  { value: "complete", label: "Complete" },
];

type SimulatorSetPickerProps = {
  label: string;
  setNumber: string;
  setName: string | null;
...
export function SimulatorSetPicker({
  label,
  setNumber,
  setName,
  condition,
  ...
  return (
    <div className="space-y-3">
      ...
      {setName && (
        <p className="text-xs text-zinc-500 truncate">{setName}</p>
      )}
      <div className="flex gap-2">
        {CONDITIONS.map((c) => (
          <button
            ...
            className={`touch-target flex-1 rounded-lg border px-3 py-2 text-xs font-semibold transition ${
              condition === c.value
